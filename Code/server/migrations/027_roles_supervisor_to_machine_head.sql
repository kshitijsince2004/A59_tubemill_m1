...
-- Affected user_ids (snapshot at migrate time): users with role_code = 'SUPERVISOR'
-- are re-pointed; those with process APPROVE but empty machine_access get all machines
-- for their APPROVE processes so machine-scoped approve does not lock them out.
...
-- Optional: bump process grants that were WRITE under supervisor personas to APPROVE for MH.
...
-- Backfill machine_access for MACHINE_HEAD users who have process APPROVE but no machines.
