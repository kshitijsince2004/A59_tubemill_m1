...
export async function getConsumable(code: string) {
...
export function isChangeDue(row: {
  cumulative_tonnage_mt?: string | number | null;
  replace_threshold_mt?: string | number | null;
  cumulative_uses?: string | number | null;
  replace_threshold_uses?: string | number | null;
}): boolean {
...
export async function applyRunUsage(runId: string): Promise<{ changeDue: boolean; code: string | null }> {
  const run = await getRun(runId);
  if (!run) throw new Error('Run not found');
  if (run.status !== 'DRAFT') throw new Error('Consumable usage can only be applied on DRAFT submit');

  const setup = await queryOne<{ work_coil_id: string | null }>(
  ...
  const updated = await queryOne<{
    cumulative_tonnage_mt: string;
    cumulative_uses: string;
    replace_threshold_mt: string | null;
    replace_threshold_uses: string | null;
  }>(
...
export async function recordInspection(
  ...
  await query(
    ...
    [
      ...
      code,
      runId ?? null,
      Number((row as { cumulative_tonnage_mt: string }).cumulative_tonnage_mt ?? 0),
      Number((row as { cumulative_uses: string }).cumulative_uses ?? 0),
      visualInspection,
      action,
    ],
  );
...
export async function getUsageHistory(code: string) {
  return query(
