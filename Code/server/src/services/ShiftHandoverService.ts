...
export async function openShift(millCode: string, shiftCode: string) {
...
export async function shiftBoundary(millCode: string, nextShiftCode: string) {
  ...
  const openRuns = await query<{ id: string }>(
  ...
  const openStoppages = await query<{ id: string }>(
  ...
  for (const id of runIds) {
    await query(
      ...
      [id, prev?.id ?? null, (next as { id: string })?.id],
    );
  }
...
export async function listShifts(millCode: string) {
  return query(
