...
export async function addDefect(runId: string, form: M1TmDefectForm, createdBy?: string) {
...
export async function listDefects(runId: string) {
...
export async function addArcWeld(form: M1TmArcWeldForm) {
  const coil = await queryOne<{ id: string; run_id: string }>(
...
export async function listArcWelds(runId: string) {
  return query(`SELECT * FROM txn.tm_arcweld_log WHERE run_id = $1 ORDER BY created_at DESC`, [runId]);
}

export async function addEdgeMill(runId: string, form: M1TmEdgeMillForm) {
  const row = await queryOne<{ id: string }>(
...
export async function listEdgeMills(runId: string) {
  return query(`SELECT * FROM txn.prod_tm_edgemill WHERE run_id = $1 ORDER BY created_at DESC`, [runId]);
}

export async function manualStop(runId: string, form: M1TmManualStopForm) {
  const run = await getRun(runId);
  if (!run) throw new Error('Run not found');
  ...
  if (!open) {
    await query(
      ...
      [
        ...
        runId,
        run.millCode,
        form.stoppageCode ?? 'TM-06',
        form.reason ?? 'Manual stop',
        form.remark ?? null,
      ],
    );
  } else if (form.stoppageCode) {
    await query(
      `UPDATE txn.stoppage_entry SET
       ...
       WHERE id = $1`,
      [open.id, form.stoppageCode, form.reason ?? null, form.remark ?? null],
    );
  }
  return getRun(runId);
}
